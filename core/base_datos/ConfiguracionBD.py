...
class MODELO_PRODUCTO(BASE):
    ...
    SUCURSALES = relationship("MODELO_SUCURSAL", secondary="PRODUCTO_SUCURSAL", back_populates="PRODUCTOS")
    EXTRAS = relationship("MODELO_EXTRA", secondary="PRODUCTO_EXTRA", back_populates="PRODUCTOS")
...
class MODELO_SUCURSAL(BASE):
    ...
    PRODUCTOS = relationship("MODELO_PRODUCTO", secondary="PRODUCTO_SUCURSAL", back_populates="SUCURSALES")
...
class MODELO_EXTRA(BASE):
    ...
    PRODUCTOS = relationship("MODELO_PRODUCTO", secondary="PRODUCTO_EXTRA", back_populates="EXTRAS")
...
class MODELO_PEDIDO(BASE):
    ...
    ESTADO = Column(String(50), default='pendiente')  # pendiente, confirmado, preparado, entregado
    ...
    DETALLES = relationship("MODELO_DETALLE_PEDIDO", back_populates="PEDIDO", cascade="all, delete-orphan")
...
def OBTENER_RUTA_BD():
    if os.name == 'nt':  # Windows
        ...
        # Para entornos web/móvil devolvemos una ruta relativa absoluta
        # y evitamos crear directorios vacíos más abajo.
        return os.path.abspath('app_segura.db')

RUTA_BD = OBTENER_RUTA_BD()
# Crear el directorio sólo si la ruta tiene un dirname no vacío.
...
MOTOR = create_engine(f'sqlite:///{RUTA_BD}', echo=False, connect_args={"check_same_thread": False})
...
async def INICIALIZAR_BASE_DATOS():
    ...
    try:
        ...
        with MOTOR.connect() as conn:
            # Verificar y añadir columnas faltantes a USUARIOS
            result = conn.execute(text("PRAGMA table_info(USUARIOS)"))
            columns = [row[1] for row in result.fetchall()]
            
            if 'FOTO_PERFIL' not in columns:
                conn.execute(text("ALTER TABLE USUARIOS ADD COLUMN FOTO_PERFIL VARCHAR(500)"))
            ...
            if 'VERIFICADO' not in columns:
                conn.execute(text("ALTER TABLE USUARIOS ADD COLUMN VERIFICADO BOOLEAN DEFAULT 0"))
                logger.info("Columna VERIFICADO añadida a USUARIOS")
            if 'TOKEN_RESET' not in columns:
                conn.execute(text("ALTER TABLE USUARIOS ADD COLUMN TOKEN_RESET VARCHAR(255)"))
                logger.info("Columna TOKEN_RESET añadida a USUARIOS")
            if 'TOKEN_RESET_EXPIRA' not in columns:
                conn.execute(text("ALTER TABLE USUARIOS ADD COLUMN TOKEN_RESET_EXPIRA DATETIME"))
                logger.info("Columna TOKEN_RESET_EXPIRA añadida a USUARIOS")
            if 'ULTIMA_CONEXION' not in columns:
                conn.execute(text("ALTER TABLE USUARIOS ADD COLUMN ULTIMA_CONEXION DATETIME"))
                logger.info("Columna ULTIMA_CONEXION añadida a USUARIOS")
            
            # Verificar y añadir IMAGEN a PRODUCTOS
            result = conn.execute(text("PRAGMA table_info(PRODUCTOS)"))
            columns = [row[1] for row in result.fetchall()]
            if 'IMAGEN' not in columns:
                conn.execute(text("ALTER TABLE PRODUCTOS ADD COLUMN IMAGEN VARCHAR(500)"))
                logger.info("Columna IMAGEN añadida a PRODUCTOS")
            if 'TIPO' not in columns:
                # Tipo de producto: 'gaseosa' por defecto
                conn.execute(text("ALTER TABLE PRODUCTOS ADD COLUMN TIPO VARCHAR(50) DEFAULT 'gaseosa'"))
                logger.info("Columna TIPO añadida a PRODUCTOS")
            
            # Verificar y añadir columnas faltantes a PEDIDOS
            result = conn.execute(text("PRAGMA table_info(PEDIDOS)"))
            columns = [row[1] for row in result.fetchall()]
            
            if 'SUCURSAL_ID' not in columns:
                conn.execute(text("ALTER TABLE PEDIDOS ADD COLUMN SUCURSAL_ID INTEGER REFERENCES SUCURSALES(ID)"))
                logger.info("Columna SUCURSAL_ID añadida a PEDIDOS")
            if 'TIPO' not in columns:
                conn.execute(text("ALTER TABLE PEDIDOS ADD COLUMN TIPO VARCHAR(20) DEFAULT 'delivery'"))
            ...
            if 'QR_PAGO' not in columns:
                conn.execute(text("ALTER TABLE PEDIDOS ADD COLUMN QR_PAGO VARCHAR(1000)"))
            ...
            if 'FECHA_CONFIRMACION' not in columns:
                conn.execute(text("ALTER TABLE PEDIDOS ADD COLUMN FECHA_CONFIRMACION DATETIME"))
    ...
    with SESION_FACTORY() as sesion:
        # Eliminar usuarios y sesiones existentes (operación destructiva)
        try:
        ...
        ROLES_DEFECTO = [
            ...
            MODELO_ROL(NOMBRE=ROLES.ATENCION, DESCRIPCION="Personal de atención al cliente"),
            ...
            MODELO_ROL(NOMBRE=ROLES.CLIENTE, DESCRIPCION="Cliente / usuario del servicio")
        ]
        ...
        # Crear usuarios por defecto (contraseñas simples de seed)
        try:
            import bcrypt
            ...
            # crear usuarios por rol con emails terminando en .com
        ...
        try:
            productos_defecto = [
                MODELO_PRODUCTO(NOMBRE='Hamburguesa Clásica', DESCRIPCION='Carne, queso, lechuga, tomate', PRECIO=599, IMAGEN='assets/hamburguesa.jpg'),
                MODELO_PRODUCTO(NOMBRE='Papas Fritas', DESCRIPCION='Papas crujientes', PRECIO=199, IMAGEN='assets/papas.jpg'),
                MODELO_PRODUCTO(NOMBRE='Bebida Gaseosa', DESCRIPCION='350ml', PRECIO=149, IMAGEN='assets/bebida.jpg')
            ]
            for prod in productos_defecto:
                existe_p = sesion.query(MODELO_PRODUCTO).filter_by(NOMBRE=prod.NOMBRE).first()
                if not existe_p:
                    sesion.add(prod)
            sesion.commit()
        except Exception as e:
        ...
        try:
            sucursales_defecto = [
                MODELO_SUCURSAL(NOMBRE='Sucursal Centro', DIRECCION='Calle Principal 123'),
                MODELO_SUCURSAL(NOMBRE='Sucursal Norte', DIRECCION='Av. Norte 456')
            ]
            for suc in sucursales_defecto:
                existe_s = sesion.query(MODELO_SUCURSAL).filter_by(NOMBRE=suc.NOMBRE).first()
                if not existe_s:
                    sesion.add(suc)
            sesion.commit()
        except Exception as e:
        ...
        try:
            ...
            for prod in productos:
                for suc in sucursales:
                    # Asignar todos los productos a todas las sucursales por defecto
        ...
        # Crear extras por defecto
        try:
            extras_defecto = [
                MODELO_EXTRA(NOMBRE='Salsa Extra', DESCRIPCION='Salsa adicional', PRECIO_ADICIONAL=50),
                MODELO_EXTRA(NOMBRE='Queso Extra', DESCRIPCION='Queso adicional', PRECIO_ADICIONAL=100)
            ]
            for ext in extras_defecto:
                existe_e = sesion.query(MODELO_EXTRA).filter_by(NOMBRE=ext.NOMBRE).first()
                if not existe_e:
                    sesion.add(ext)
            sesion.commit()
        except Exception as e:
