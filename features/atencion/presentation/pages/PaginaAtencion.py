...
from core.base_datos.ConfiguracionBD import OBTENER_SESION, MODELO_PEDIDO, MODELO_CAJA, MODELO_SUCURSAL
...
class PaginaAtencion(ft.Column):
    ...
    def _CONSTRUIR(self):
        ...
        self._DROPDOWN_SUCURSAL = ft.Dropdown(label="Seleccionar Sucursal", on_select=self._CAMBIAR_SUCURSAL)
        ...
        self.controls = [
            ft.Container(content=ft.Column([
                ...
                ft.Row([self._TEXTO_CAJA, self._BOTON_CAJA], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
        ...
        # Intentar registrar callback WebSocket cuando la conexión esté disponible
        import asyncio
    ...
    async def _INICIALIZAR_WEBSOCKET(self):
        ...
        # Esperar hasta que la conexión para este usuario exista
        while True:
            ...
            if cliente:
                async def _on_msg(mensaje):
                    try:
                        ...
                        if tipo in ('nuevo_pedido', 'pedido_confirmado', 'pedido_pagado'):
                            # si pertenece a la sucursal mostrada, refrescar
    ...
    def _VERIFICAR_CAJA(self):
        if not self._SUCURSAL_SELECCIONADA:
            return
        sesion = OBTENER_SESION()
        caja = sesion.query(MODELO_CAJA).filter_by(
        ...
        if caja:
            self._TEXTO_CAJA.value = f"Caja abierta - Monto inicial: {caja.MONTO_INICIAL} Bs"
    ...
    async def _MANEJAR_CAJA(self):
        ...
        if self._CAJA_ACTIVA:
            # Cerrar caja: volver a obtener la instancia ligada a la sesión actual
            ...
            pedidos_hoy = sesion.query(MODELO_PEDIDO).filter(
    ...
    async def _CARGAR_PEDIDOS(self):
        ...
        pedidos = sesion.query(MODELO_PEDIDO).filter(
            MODELO_PEDIDO.SUCURSAL_ID == self._SUCURSAL_SELECCIONADA,
            MODELO_PEDIDO.FECHA_CREACION >= hoy
        ).order_by(MODELO_PEDIDO.FECHA_CREACION.desc()).all()
        ...
        for p in pedidos:
            ...
            estado_color = ft.Colors.GREEN_600 if p.ESTADO == 'confirmado' else ft.Colors.ORANGE_400 if p.ESTADO == 'preparado' else ft.Colors.RED_600
            fila = ft.Row([
                ft.Column([
                ...
                ft.Column([
                    ft.Button("Confirmar Pago", on_click=lambda e, ped=p: asyncio.create_task(self._CONFIRMAR_PAGO(ped))),
                    ft.Button("Imprimir Recibo", on_click=lambda e, ped=p: self._IMPRIMIR_RECIBO(ped))
                ])
    ...
    def _IMPRIMIR_RECIBO(self, PEDIDO):
        # Simular impresión (en producción usar biblioteca de impresión)
        recibo = f"""
Recibo #{PEDIDO.ID}
Fecha: {PEDIDO.FECHA_CREACION}
Total: {PEDIDO.MONTO_TOTAL} Bs
