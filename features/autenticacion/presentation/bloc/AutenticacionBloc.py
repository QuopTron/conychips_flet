...
class AutenticacionBloc:
        ...
        try:
            ...
            if RESULTADO["EXITO"]:
                ...
                if RESULTADO.get("REQUIERE_2FA"):
                    self._EMITIR_ESTADO(EstadoRequiereSegundoFactor(
                        USUARIO_ID=RESULTADO["USUARIO"].ID,
                        METODO=RESULTADO.get("METODO_2FA", "email")
                    ))
                else:
                    self._EMITIR_ESTADO(EstadoAutenticado(
    ...
    async def _MANEJAR_REGISTRARSE(self, EVENTO: EventoRegistrarse):
        ...
        try:
            ...
            if RESULTADO["EXITO"]:
                self._EMITIR_ESTADO(EstadoRegistroExitoso(
                    EMAIL=EVENTO.EMAIL
                ))
            else:
                self._EMITIR_ESTADO(EstadoError(
                    ERROR=RESULTADO["ERROR"],
                    CODIGO=RESULTADO.get("CODIGO", 400)
                ))
