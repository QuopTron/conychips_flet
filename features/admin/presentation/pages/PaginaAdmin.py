...
class PaginaAdmin(ft.Column):
    ...
    def _CONSTRUIR(self):
        ...
        self._BOTON_VER_USUARIOS = ft.Button(
            "Gestionar Usuarios", on_click=self._VER_USUARIOS
        )
        self._BOTON_VER_PRODUCTOS = ft.Button(
            "Gestionar Productos", on_click=self._VER_PRODUCTOS
        )
        self._BOTON_VER_SUCURSALES = ft.Button(
            "Gestionar Sucursales", on_click=self._VER_SUCURSALES
        )
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Row(
                            [
                                self._STATS_USUARIOS,
                                self._STATS_PEDIDOS,
                                self._STATS_GANANCIAS,
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_AROUND,
                        ),
                        ft.Text(
                            "Estadísticas por Rol", size=18, weight=ft.FontWeight.BOLD
                        ),
                        self._GRAFICO_ROLES,
                        ft.Text(
                        ...
                        ft.Row(
                            [
                                self._BOTON_VER_USUARIOS,
                                self._BOTON_VER_PRODUCTOS,
                                self._BOTON_VER_SUCURSALES,
                            ]
                        ),
    ...
    async def _OBTENER_STATS(self):
        ...
        cajas_hoy = (
            sesion.query(MODELO_CAJA).filter(MODELO_CAJA.FECHA_APERTURA >= hoy).all()
        )
        ganancias = sum(c.GANANCIAS or 0 for c in cajas_hoy)

        self._STATS_USUARIOS.value = (
            f"Usuarios totales: {sesion.query(MODELO_USUARIO).count()}"
        )
        ...
        for s in sucursales:
            pedidos_suc = (
                sesion.query(MODELO_PEDIDO).filter_by(SUCURSAL_ID=s.ID).count()
            )
            self._GRAFICO_SUCURSALES.controls.append(
                ft.Text(f"{s.NOMBRE}: {pedidos_suc} pedidos")
            )
    ...
    def _VER_USUARIOS(self, e):
        ...
        for u in usuarios:
            ...
            filas.append(
                ft.Row(
                    [
                        ft.Text(u.NOMBRE_USUARIO),
                        ft.Text(u.EMAIL, size=12, color=ft.Colors.GREY_600),
                        ft.Icon(ft.Icons.FIBER_MANUAL_RECORD, color=color),
                    ],
