"""
Vista de gestión de extras con CRUD completo en popups
"""
import flet as ft
...
class VistaExtras(VistaBase):
    ...
    def _cargar_vista(self):
        boton_nuevo = ft.ElevatedButton("➕ Nuevo Extra", icon=ft.Icons.ADD_CIRCLE, on_click=self._abrir_popup_crear, bgcolor=COLORES.PRIMARIO, color=COLORES.TEXTO_BLANCO)
    ...
    def _abrir_popup_crear(self, e):
        campo_nombre = ft.TextField(label="Nombre", prefix_icon=ft.Icons.ADD_CIRCLE)
        campo_descripcion = ft.TextField(label="Descripción", multiline=True)
        campo_precio = ft.TextField(label="Precio Adicional (centavos)", prefix_icon=ft.Icons.ATTACH_MONEY, keyboard_type=ft.KeyboardType.NUMBER)
    ...
    def _abrir_popup_editar(self, item):
        campo_nombre = ft.TextField(label="Nombre", value=item.NOMBRE, prefix_icon=ft.Icons.ADD_CIRCLE)
